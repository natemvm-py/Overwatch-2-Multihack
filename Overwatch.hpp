...
namespace OW {
		...
		while (Config::doingentity == 1) {
			...
			for (int i = 0; i < ow_entities.size(); i++) {
				...
				if (entity.AngleBase)
				{
					...
					if (GetHeroEngNames(entity.HeroID, entity.LinkBase) != (u8"Unknown")) {
	...
	inline void viewmatrix_thread_debug() {
		try {
			std::cerr << "[INFO] Starting viewmatrix_thread.\n";

			while (true) {
				std::cerr << "[DEBUG] Reading viewMatrixVal.\n";
				auto viewMatrixVal = (SDK->RPM<uint64_t>(SDK->dwGameBase + offset::Address_viewmatrix_base) ^ offset::offset_viewmatrix_xor_key) - offset::offset_viewmatrix_xor_key2;

				std::cerr << "[DEBUG] Reading WindowSize.\n";
				Vector2 WindowSize = SDK->RPM<Vector2>(viewMatrixVal + 0x41C);

				static RECT TempRect = { NULL };
				static POINT TempPoint;

				std::cerr << "[DEBUG] Getting client rect and screen position.\n";
				GetClientRect(SDK->Windowsizehd, &TempRect);
				ClientToScreen(SDK->Windowsizehd, &TempPoint);
				...
				std::cerr << "[DEBUG] Reading viewMatrix and viewMatrix_xor.\n";
				viewMatrix = SDK->RPM<Matrix>(viewMatrixPtr);
				viewMatrix_xor = SDK->RPM<Matrix>(viewMatrix_xor_ptr);

				std::cerr << "[DEBUG] Sleeping for 5 ms.\n";
				Sleep(5);
			}
		}
		catch (...) {
			std::cerr << "[ERROR] Exception caught in viewmatrix_thread.\n";
		}
	}
	...
	inline void overlay_thread() {
		__try {
			...
			if(ico234 == nullptr)
				ico234 = io.Fonts->AddFontFromMemoryTTF((void*)lemonmilk, sizeof(lemonmilk), 65.f, NULL, io.Fonts->GetGlyphRangesCyrillic());
			...
			while (FindWindowA(skCrypt("TankWindowClass"), NULL))
			{
				...
				if (Config::Menu)
				{
					...
					bool _visible = true;
					if (ImGui::Begin(skCrypt(u8"main_window"), &_visible, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoTitleBar)) {
						ImGui::Spacing();
						ImGui::PushFont(ico);
						...
						ImGui::Spacing();
						ImGui::SameLine();
						ImGui::Text(skCrypt("         Overwatch 2 Multihack"));
						ImGui::PopStyleColor();
						ImGui::PopFont();
						ImGui::PushFont(ico);
						ImGui::Spacing();
						...
						if (tab_index == 1) {
							...
							if (subindex == 2) {
								ImGui::BulletText(u8"Secondary Aim is not available with silent and legit");
								ImGui::Spacing();
								ImGui::Spacing();
								ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(1.f, 0.f, 0.f, 1.0f));
						...
						if (tab_index == 5) {
							...
							if (ImGui::Button(u8"Manual Save"))
							{
								...
								ImGui::InsertNotification({ ImGuiToastType_Success, 3000, skCrypt(u8"Manual save successfull"), "" });
							}
							...
							if (ImGui::Button(u8"Reboot thread")) {
								...
								Sleep(51);
								ImGui::InsertNotification({ ImGuiToastType_Success, 3000, skCrypt(u8"Reboot Successfull"), "" });
							}
							ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(colora5 / 255.f, colorb5 / 255.f, colorc5 / 255.f, 255.f / 255.f));
							ImGui::Text(u8"Overwatch 2 Multihack. Github @ natemvm-py", (expiretime - nowtime) / 60 / 60 / 24, ((expiretime - nowtime) / 60 / 60) % 24, ((expiretime - nowtime) / 60) % 60, (expiretime - nowtime) % 60);
							if (Config::loginornot && (expiretime - nowtime) < 0) abort();
							ImGui::Text(u8"FPS: %.1f FPS", 1000.0f / ImGui::GetIO().Framerate);
							ImGui::PopStyleColor();
							ImGui::End();
						}
					}
				...
				if (entities.size() > 0) {
					...
					if (Config::triggerbot && GetAsyncKeyState(Config::aim_key)) {
						...
						if (vec != Vector3(0, 0, 0) && !(entities[Config::Targetenemyi].skill2act && entities[Config::Targetenemyi].HeroID == eHero::HERO_GENJI)) {
							auto local_angle = SDK->RPM<Vector3>(SDK->g_player_controller + 0x11A0);
						...
						while (GetAsyncKeyState(Config::aim_key) && !Config::reloading)
						{
							...
							if (vec != Vector3(0, 0, 0) && !(entities[Config::Targetenemyi].skill2act && entities[Config::Targetenemyi].HeroID == eHero::HERO_GENJI) && ((!entities[Config::Targetenemyi].imort && !entities[Config::Targetenemyi].barrprot) || Config::switch_team)) {
								...
								if (Target != Vector3(0, 0, 0)) {
									...
									if (Config::Rage) SDK->WPM<Vector3>(SDK->g_player_controller + 0x11A0, vec_calc_target);
									else
										SDK->WPM<Vector3>(SDK->g_player_controller + 0x11A0, Target);
								...
								if (Target != Vector3(0, 0, 0)) {
									...
									if (Config::Rage) {
										if (Config::fakesilent) {
											Vector3 orangle = SDK->RPM<Vector3>(SDK->g_player_controller + 0x11A0);
											SDK->WPM<Vector3>(SDK->g_player_controller + 0x11A0, vec_calc_target);
											if (Config::lockontarget)
												SDK->WPM<float>(GetSenstivePTR(), 0);
											SetKey(0x1);
											Sleep(25);
											if (Config::lockontarget)
												SDK->WPM<float>(GetSenstivePTR(), origin_sens);
											Config::shooted = true;
											SDK->WPM<Vector3>(SDK->g_player_controller + 0x11A0, orangle);
											continue;
										}
										else {
											SDK->WPM<Vector3>(SDK->g_player_controller + 0x11A0, vec_calc_target);
						...
						while (Config::GenjiBlade && (Config::Qtime - Config::Qstarttime) <= 7000) {
							...
							if (vec != Vector3(0, 0, 0)) {
								...
								if (Target != Vector3(0, 0, 0)) {
									...
									{
										if (Config::Rage) SDK->WPM<Vector3>(SDK->g_player_controller + 0x11A0, vec_calc_target);
										else SDK->WPM<Vector3>(SDK->g_player_controller + 0x11A0, Target);
									}
	...
	inline void configsavenloadthread() {
		...
		if (Config::lastheroid == -2) {
			ImGui::InsertNotification({ ImGuiToastType_Success, 8000, skCrypt(u8"Internal Loaded\nWelcome！"), "" });
			Config::lastheroid = 0;
		}
		while (1) {
			if (!Config::Menu) {
