	...
	inline uintptr_t DecryptComponent(uint64_t Parent, uint8_t index)
	{
		__try {
			if (Parent) {
				...
				uint64_t var_qword = SDK->RPM<uint64_t>(SDK->dwGameBase + 0x349D418);
				uint8_t var_byte = SDK->RPM<uint8_t>(SDK->dwGameBase + 0x31D0D5C);

				uint64_t v8 = var_byte ^ SDK->RPM<uint64_t>(var_qword + 196) ^ __ROR8__(
					SDK->RPM<uint64_t>(SDK->RPM<uint64_t>(v1 + 128)
						+ 8 * (SDK->RPM<uint8_t>((unsigned int)v5 + v1 + 304)
					...
					17)
					+ 0x54B93D890DD637BCi64;

				v8 = (v8 ^ 0x9DF3836C0C43A328ui64) + 0x61D4D274C96243E8i64;

				const uintptr_t v9 = (int)-(__int64)((v2 & v6) >> v4) & ((__ROR8__(v8, 30) ^ 0xE5C25203BE9426FEui64) + 0x21F57E36BB818050i64);
				return v9;
			}
		}
